<!-- Footer -->

<!DOCTYPE html>

<html lang = "en">

    <head>
        
        <meta charset="utf-8">

        <!--Boostrap-->
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>

        <!--Icon-->

        <!-- <link rel = "icon" href = "title-icon.png"> -->

        <!--Styles-->

        <style>
        /* Font Import */
        @import url('https://fonts.googleapis.com/css2?family=Roboto&display=swap');

        section {

            padding-top:50px;
            bottom: 0;

        }

        #footer {

            left: 0;
            bottom: 0 !important; 
            width: 100%;
            background-color: #02010a;
            color: #f3ecfe;
            
        }

        #footer li a {

            color: #f3ecfe;
            text-decoration: none !important;
            font-family: 'Roboto', sans-serif;
        }

        #footer li a:hover {

            color: #d5c3e9  !important;
            text-decoration: none !important;
            transition:all .5s;

        }

        .nav-links {
            
            padding-bottom: 20px;
            font-size:12pt;
        }

        .footer-about {

            bottom:0;
            padding-top:60px;
            padding-bottom:20px;
            font-size:10pt;

            
        }

        .footer-about a {

            color:#ffff;

        }

        .footer-about a:hover {

            text-decoration:none;
            font-weight:bold;
            transition:all .5s;
            color:#ffff;

        }




        </style>




    </head>


    <body>

    
    <!--Footer-->

        <section id="footer">
            <div class="container">

                <div class="row text-sm-left text-md-left text-lg-left">

                    <!--Logo-->
                    <div class="col-sm-4 col-md-4 col-lg-4">

                            <img src="img/devslounge-logo.png" width="200" height="58" alt="logo">  

                    </div>

                    <!--Nav Links-->
                    <div class="col-sm-4 col-md-4 col-lg-4">

                            <ul class="list-unstyled quick-links">
                                <li><a href="home.php">Home</a></li>
                                <li><a href="jobpostings.php">Job Postings</a></li>

    
                            </ul>

                    </div>

                    <div class="col-sm-4 col-md-4 col-lg-4">
   
                            <ul class="list-unstyled quick-links">
                                <li><a href="contact.php">Contact Us</a></li>
                                <li><a href="about.php">About</a></li>

                            </ul>

                    </div>


                </div>

            

                <!--Company Summary-->
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-center text-white">
                        <div class="footer-about">
                            DevsLounge is a remote job platform for developers around the world.
                            <p class="h6">© All Rights Reserved. Group Luxembourg. </p>
                            <p class="h6"><a href="https://storyset.com/work">Illustrations by Freepik Storyset</a></p>
                        </div>
                    </div>
                  
                </div>
                    
            </div>



        </section>



    </body>

</html>